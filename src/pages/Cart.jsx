...
const Cart = ({ itemCount = 0 }) => {
    ...
    return (
        <main className="container-custom scroll-smooth pt-2.5 pb-10 md:pt-3 md:pb-12">
            <section className="mb-8 text-3xl font-bold md:text-4xl">Giỏ hàng ({cart?.length}) </section>
                    ...
                    >
                        <thead className="">
                            <tr className="hidden text-left">
                                ...
                                <th className="table-cell">Tổng cộng</th>
                            </tr>
                        </thead>
                        <tbody className="">
                                        ...
                                        <div className="flex w-full flex-1 items-center gap-3">
                                            ...
                                            <div className="flex items-center">
                                                <img
                                                    src={
                                                        ...
                                                        Array.isArray(item.productVariant.images) &&
                                            ...
                                            <div className="flex flex-1 flex-col justify-between gap-1 sm:gap-2 md:gap-2.5">
                                                <div className="flex flex-1 flex-col">
                                                    <span className="text-lg font-semibold md:text-xl">{item.productName || "Tên sản phẩm"}</span>
                                        ...
                                        <div className="flex w-fit items-center justify-between gap-3 rounded-full border px-5 py-2 font-bold">
                                            <button
                                                className={`cursor-pointer text-lg ${
                                                    (localQuantities[item.id] ?? item.quantity) <= 1 ? "text-gray-400" : ""
                                                }`}
                                                disabled={(localQuantities[item.id] ?? item.quantity) <= 1 || loading}
                                                onClick={(val) => debouncedUpdateCart(item, (localQuantities[item.id] ?? item.quantity) - 1)}
                                            >
                                                -
                                            </button>
                                            <QuantityInput
                                            ...
                                            <button
                                                ...
                                                disabled={
                                                    loading ||
                                                    !item.productVariant ||
                                                    (localQuantities[item.id] ?? (item.quantity >= item.productVariant?.stockQty || 0))
                                                }
                                                onClick={() => {
                                                    // ✅ THÊM: Kiểm tra trước khi tăng
                            ...
                            {cart && cart.length === 0 && (
                                <tr>
                                    ...
                                    >
                                        Giỏ hàng của bạn đang trống. Hãy bắt đầu mua sắm ngay!
                                    </td>
                                </tr>
                            )}
                        </tbody>
                ...
                <div className="col-span-12 md:col-span-4 md:pr-5">
                    ...
                    >
                        <div className="mb-3 flex flex-col gap-2 pb-5">
                            <p className="font-semibold text-[#0d8756]"> Bạn được miễn phí vận chuyển. </p>
                            <ProgressBar />
                        </div>
                        <div className="grid gap-6">
                            {/* Cart Addon */}
                            <div className="rounded-full bg-gray-200 px-5 py-4 text-lg font-bold">Ghi chú đơn hàng</div>
                            ...
                            {/* <div className="flex cursor-pointer justify-between border-b-2 border-dashed pb-3">
                                <Ticket
                                    ...
                                    size={25}
                                />
                                <p className="font-semibold text-gray-500">Nhận khuyến mãi của bạn</p>
                            </div> */}
                            <div className="grid gap-1">
                                <div className="flex justify-between text-lg font-bold whitespace-nowrap lg:text-xl">
                                    <span className="">Tổng tiền ước tính</span>
                                    <span>{formatVNDForDisplay(cartTotal)}</span>
                                </div>
                                <div className="">Phí vận chuyển sẽ được tính tại thanh toán</div>
                            </div>
